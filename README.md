...
- name: Delete releases and workflows runs
  ...
  with:
    ...
    releases_keep_keyword: "v0/kernel_"
    delete_tags: true
...
| ----------------------- | ---------- | ---------------------------------------- |
| `delete_releases`       | Optional<br />可选项 | Whether to delete Releases (options: `true`/`false`). Default: `false`.<br />是否删除 Releases（选项：`true`/`false`），默认为 `false`。 |
...
| `delete_workflows`      | Optional<br />可选项 | Whether to delete Workflow run records (options: `true`/`false`). Only completed runs are processed. Default: `false`.<br />是否删除 Workflow 运行记录（选项：`true`/`false`）。仅处理已完成（completed）的运行记录。默认为 `false`。 |
